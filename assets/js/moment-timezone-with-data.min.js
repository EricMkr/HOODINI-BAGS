!function(M, z) {
   "use strict";
   "object" == typeof module && module.exports ? module.exports = z(require("moment")) : "function" == typeof define && define.amd ? define(["moment"], z) : z(M.moment)
}(this, function(O) {
   "use strict";
   void 0 === O.version && O.default && (O = O.default);
   var z, W = {}, A = {}, c = {}, d = {}, R = {}, M = (O && "string" == typeof O.version || C("Moment Timezone requires Moment.js. See https://momentjs.com/timezone/docs/#/use-it/browser/"),
   O.version.split(".")), b = +M[0], p = +M[1];
   function q(M) {
   ...
   function L(M, z) {
       for (var b = [], p = 0; p < z.length; p++)
           b[p] = M[z[p]];
       return b
   }
   function n(M) {
       for (var M = M.split("|"), z = M[2].split(" "), b = M[3].split(""), p = M[4].split(" "), O = (a(z),
       a(b),
       a(p),
       p), A = b.length, c = 0; c < A; c++)
   ...
   function N(M) {
       var z = M.toTimeString()
         , b = z.match(/\([a-z ]+\)/i);
       "GMT" === (b = b && b[0] ? (b = b[0].match(/[A-Z]/g)) ? b.join("") : void 0 : (b = z.match(/[A-Z]{3,5}/g)) ? b[0] : void 0) && (b = void 0),
       this.at = +M,
       this.abbr = b,
   ...
   function i() {
       for (var M, z, b, p = (new Date).getFullYear() - 2, O = new N(new Date(p,0,1)), A = O.offset, c = [O], q = 1; q < 48; q++)
           (b = new Date(p,q,1).getTimezoneOffset()) !== A && (M = function(M, z) {
               for (var b; b = 6e4 * ((z.at - M.at) / 12e4 | 0); )
                   (b = new N(new Date(M.at + b))).offset === M.offset ? M = b : z = b;
               return M
   ...
   function u(M, z) {
       return M.offsetScore !== z.offsetScore ? M.offsetScore - z.offsetScore : M.abbrScore !== z.abbrScore ? M.abbrScore - z.abbrScore : M.zone.population !== z.zone.population ? z.zone.population - M.zone.population : z.zone.name.localeCompare(M.zone.name)
   }
   function e() {
       try {
           ...
           if (M && 3 < M.length) {
               ...
               if (z)
                   return z;
               C("Moment Timezone found " + M + " from the Intl api, but did not have that data loaded.")
           }
       } catch (M) {}
       for (var b, p, O = i(), A = O.length, c = function(M) {
           for (var z, b, p, O = M.length, A = {}, c = [], q = {}, o = 0; o < O; o++)
               if (b = M[o].offset,
               !q.hasOwnProperty(b)) {
                   for (z in p = R[b] || {})
           ...
           for (o in A)
               A.hasOwnProperty(o) && c.push(d[o]);
           return c
       }(O), q = [], o = 0; o < c.length; o++) {
           for (b = new X(t(c[o])),
           p = 0; p < A; p++)
               b.scoreOffsetAt(O[p]);
           q.push(b)
       ...
       return q.sort(u),
       0 < q.length ? q[0].zone.name : void 0
   }
   function r(M) {
   ...
   function t(M, z) {
       M = r(M);
       var b = W[M];
       return b instanceof f ? b : "string" == typeof b ? (b = new f(b),
       W[M] = b) : A[M] && z !== t && (z = t(A[M], t)) ? ((b = W[M] = new f)._set(z),
       b.name = d[M],
       b) : null
   }
   function l(M) {
   ...
   function S(M) {
       ...
       return !O.isMoment(M) && E(z) && (M = t(b)) && z.add(M.parse(z), "minutes"),
       z.tz(b),
       z
   }
   (b < 2 || 2 == b && p < 6) && C("Moment Timezone requires Moment.js >= 2.6.0. You are using Moment.js " + O.version + ". See momentjs.com"),
   f.prototype = {
       _set: function(M) {
           this.name = M.name,
           this.abbrs = M.abbrs,
       ...
       parse: function(M) {
           for (var z, b, p, O = +M, A = this.offsets, c = this.untils, q = c.length - 1, o = 0; o < q; o++)
               if (z = A[o],
               b = A[o + 1],
               p = A[o && o - 1],
               z < b && S.moveAmbiguousForward ? z = b : p < z && S.moveInvalidForward && (z = p),
               O < c[o] - 6e4 * z)
                   return A[o];
           return A[q]
       },
   ...
   S.zonesForCountry = function(M, z) {
       var b;
       return b = (b = M).toUpperCase(),
       (M = c[b] || null) ? (b = M.zones.sort(),
       z ? b.map(function(M) {
           return {
               name: M,
   ...
   O.updateOffset = function(M, z) {
       var b, p = O.defaultZone;
       void 0 === M._z && (p && E(M) && !M._isUTC && M.isValid() && (M._d = O.utc(M._a)._d,
       M.utc().add(p.parse(M), "minutes")),
       M._z = p),
       M._z && (p = M._z.utcOffset(M),
       Math.abs(p) < 16 && (p /= 60),
       void 0 !== M.utcOffset ? (b = M._z,
       M.utcOffset(-p, z),
       M._z = b) : M.zone(p, z))
   }
   ,
   M.tz = function(M, z) {
       if (M) {
           ...
           return this._z = t(M),
           this._z ? O.updateOffset(this, z) : C("Moment Timezone has no data for " + M + ". See http://momentjs.com/timezone/docs/#/data-loading/."),
           this
       }
       if (this._z)
           return this._z.name
   ...
   O.tz.setDefault = function(M) {
       return (b < 2 || 2 == b && p < 9) && C("Moment Timezone setDefault() requires Moment.js >= 2.9.0. You are using Moment.js " + O.version + "."),
